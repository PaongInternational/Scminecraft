#!/bin/bash
# 01b_setup_alt.sh - Skrip Setup Cadangan (Menggunakan apt)
...
echo "--- 1. Instalasi Paket Dasar (apt) ---"
apt update -y
apt upgrade -y
apt install -y tmux wget unzip curl nano jq
...
if ! command -v ngrok &> /dev/null; then
    ...
    unzip ngrok.zip
    chmod +x ngrok
    # Penempatan alternatif di $PREFIX/bin/
    mv ngrok $PREFIX/bin/
    rm ngrok.zip
    echo "Ngrok diinstal ke $PREFIX/bin/ (jalur cadangan)."
else
